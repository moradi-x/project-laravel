@extends('layoutes.template', ['title' => 'category page | ' . $category->name])
                        ...
                        <div id="filter-selector" class="flex items-center space-x-4 font-light ">
                            <a class="text-gray-500 py-4
                             {{ request('order') != 'asc' ? 'text-gray-900 border-b border-gray-900 ' : '' }} "
                                href=" {{ route('category', [ 'category' => $category->id , 'order' => 'desc']) }} ">Latest</a>

                            <a class="text-gray-500 py-4
                             {{ request('order') == 'asc' ? 'text-gray-900 border-b border-gray-900 ' : '' }} "
                                href=" {{ route('category', [ 'category' => $category->id , 'order' => 'asc']) }}">Oldest</a>
                        </div>
                    </div>
                                    ...
                                    <div class="article-body grid grid-cols-12 gap-3 mt-5 items-start">
                                        <div class="article-thumbnail col-span-4 flex items-center">
                                            <a href="{{ route('single', ['post' => $post->slug]) }}">
                                                <img class="mw-100 mx-auto rounded-xl"  src="{{ asset($post->thumbnail) }}"
                                                    alt="thumbnail">
                                            </a>
                                        </div>
