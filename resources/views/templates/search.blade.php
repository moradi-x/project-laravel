@extends('layoutes.template', ['title' => 'search for : ' . request('word')])
        ...
        <div class="w-full grid grid-cols-4 gap-10">
            <div class="md:col-span-3 col-span-4">
                <div id="posts" class=" px-3 lg:px-7 py-6">
                    <div class="my-3">
                        <h3 class="font-bold"> Result for : {{ request('word') }} </h3>
                    </div>
                    <div class="flex justify-between items-center border-b border-gray-100">
                        <div id="filter-selector" class="flex items-center space-x-4 font-light ">
                            <a class="text-gray-500 py-4
                             {{ request('order') != 'asc' ? 'text-gray-900 border-b border-gray-900 ' : '' }} "
                                href=" {{ route('search', ['order' => 'desc' , 'word' => request('word') ]) }} ">Latest</a>

                            <a class="text-gray-500 py-4
                             {{ request('order') == 'asc' ? 'text-gray-900 border-b border-gray-900 ' : '' }} "
                                href=" {{ route('search', ['order' => 'asc' , 'word' => request('word') ]) }}">Oldest</a>
                        </div>
                    </div>
