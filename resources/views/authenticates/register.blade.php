@extends('layoutes.template', ['title' => 'Register page'])
...
@section('main')
    <div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
        <div class="sm:mx-auto sm:w-full sm:max-w-sm">
            ...
            <h2 class="mt-10 text-center text-2xl/9 font-bold tracking-tight text-gray-900">Register in to WebSite</h2>
        </div>
        ...
        <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
            <form action="{{ route('register.user') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <input id="family" type="text" name="family"  autocomplete="family" value=" {{ old('family') }}"
                            class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"  />
                        @error('family')
                            <div class="text-red-500 mt-1 text-sm ">{{ $message }}</div>
                        @enderror
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm/6 font-medium text-gray-900"> Repeate Password</label>
                    <div class="mt-2">
            ...
            <p class="mt-10 text-center text-sm/6 text-gray-500">
                Your are member?
                <a href="{{ route('login') }}" class="font-semibold text-yellow-500 hover:text-yellow-900">Login to
                    WebSite</a>
            </p>
        </div>
    </div>
